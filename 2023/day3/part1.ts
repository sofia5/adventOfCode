...
const getNumbers = (lines: string[]) => {
    return lines.flatMap((line, index) => {
        ...
        return numberMatches.map(match => ({number: Number(match[0]), colIndexStart: match.index ?? 0, colIndexEnd: (match.index ?? 0) + match[0].length, rowIndex: index}))
    })
}
...
const validNumbers = numbers.filter((number) => {   
    ...
    if(symbols.get(rowIndex)?.get(colIndexStart - 1) || symbols.get(rowIndex)?.get(colIndexEnd)) {
        return true
    }

    for(let i = colIndexStart - 1; i <= colIndexEnd; i++) {
        if(symbols.get(rowIndex - 1)?.get(i) || symbols.get(rowIndex + 1)?.get(i)) {
            return true
        }   
    }
...
const sum = validNumbers.reduce((partialSum, item) => partialSum + item.number, 0)
console.log(sum)